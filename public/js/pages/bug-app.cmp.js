...
export default {
  template: `
    <section class="bug-app">
        <div class="subheader">
          ...
          <router-link to="/bug/edit" v-if="!userKnown">Add New Bug</router-link> ||
  ...
  computed: {
    userKnown() {
      userService.isUserSigned()
    }
  },
  methods: {
    goPage(diff) {
      this.filterBy.page = (this.filterBy.page + diff) % this.totalPages;
    ...
    removeBug(bugId) {
      bugService.remove(bugId).then(() => {
        ...
        this.loadBugs();
      }).catch(console.log("can't remove"))
    },
    onDownloadPDF() {
